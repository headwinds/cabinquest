...
import { addPortholeTrees } from './PortholeTreeUtils';
//import {createForestWithPortholeTrees} from
//import {updateStatus} from

const serverPath = '/';
const bSignedIn = true;
...
export const addDefaultPortholeTrees = cabinQuestUserModel => {
    console.log('addDefaultPortholeTrees: ', cabinQuestUserModel);
    const allPortholeTreesObj = getAllPortholeTrees(); // returns an object with keys
    ...
    return addPortholeTrees(allPortholeTrees, cabinQuestUserModel);
};
...
const getRSSBranch = (branch, index) => {
    ...
    const images = PortholeImageService.getImagesFromDescription(branch);
    ...
    const photoLargeUrl =
        photoUrl.indexOf('500') > -1
            ? photoUrl.replace('500', '1280')
            : photoUrl;

    const branchTitleUnescape = _.unescape(branch.title);
    const branchDescriptionUnescape = _.unescape(resultImageObj.text);
    ...
    */
    // fix title
    if (branch.meta.title === 'National Geographic Photo of the Day')
        branch.meta.title = 'National Geographic';
    else if (branch.meta.title === 'Latest Articles')
        branch.meta.title = 'Dwell';
    else if (branch.meta.title === 'Latest Items from TreeHugger')
        branch.meta.title = 'TreeHugger';
    else if (branch.meta.title.trim() === 'Polygon -  All')
        branch.meta.title = 'Polygon';
    else if (branch.meta.title.trim() === 'Behance Featured Projects')
        branch.meta.title = 'Behance';
    else if (
        branch.meta.title.trim() ===
        'Abduzeedo Design Inspiration - Design Inspiration & Tutorials'
    )
        branch.meta.title = 'Abduzeedo';
    else if (branch.meta.title.trim() === 'Latest News - Pitchfork')
        branch.meta.title = 'Pitchfork';
    else if (branch.meta.title.trim() === 'rackk and ruin')
        branch.meta.title = 'Rackk and Ruin';
    else if (branch.meta.title.trim() === 'BOILER ROOM')
        branch.meta.title = 'Boiler Room';
    else if (branch.meta.title.trim() === 'CRISTIAN ORDONEZ')
        branch.meta.title = 'Cristian Ordóñez';
    else if (branch.meta.title.trim() === 'Get Outside — Medium')
        branch.meta.title = 'Headlands';
    else if (branch.meta.title.trim() === 'ArtStation - Latest Picks')
        branch.meta.title = 'ArtStation';
    else if (
        branch.meta.title.trim() === 'Y! Sports Blogs  - Yahoo Canada Sports'
    )
        branch.meta.title = 'Yahoo Sports';
    else if (
        branch.meta.title.trim() === 'International Documentary Association'
    )
        branch.meta.title = 'Documentary';
    else if (branch.meta.title.trim() === '/Film') branch.meta.title = 'Slash';
    else if (branch.meta.title.trim() === 'Film Blog | The Guardian')
        branch.meta.title = 'The Guardian';
    else if (branch.meta.title.trim() === 'Crosscuts')
        branch.meta.title = 'Walker';
    else if (branch.meta.title.trim() === 'NBA.com: News')
        branch.meta.title = 'NBA';
    else if (branch.meta.title.trim() === 'CONTEMPORIST')
        branch.meta.title = 'Contemporist';
    else if (branch.meta.title.trim() === 'Sports - The Inquisitr News')
        branch.meta.title = 'The Inquisitr';
    else if (
        branch.meta.title.trim() ===
        'designboom | architecture & design magazine'
    )
        branch.meta.title = 'designboom';
...
export const getAllPortholeTrees = () => {
    return createAllPortholeTrees();
};

const loadTree = treeObj => {
    var serviceRoute = serverPath + '/bellwoods/trees/load';

    self.data = {
        loadTreePromise: axios.get(devPath, {
            params: serviceRoute
        })
    };
...
const loadTreeById = treeObj => {
    const serviceRoute =
        serverPath + '/bellwoods/trees/getTreeByRSSUrl/' + treeObj._id;

    self.data = {
        loadTreePromise: axios.get(serviceRoute)
    };

    return self.data;
};

const getTreeByRSSUrl = treeObj => {
    const serviceRoute =
...
const getForestFeed = (trees, bSignedIn) => {
    ...
    if (bSignedIn) {
        _.each(trees, function(tree) {
            const treePromise = parent.getTreeByRSSUrl(tree);
        ...
        _.each(trees, function(treeValue) {
            ...
            if (undefined !== currentTree) {
                const treePromise = parent.getTreeByRSSUrl(currentTree);
                treePromises.push(treePromise);
            }
        });
    }
...
const getForestFeedByTreeModels = treeModels => {
    const treePromises = [];

    const parent = this;

    _.each(treeModels, function(treeModel) {
        const treePromise = parent.getTreeByRSSUrl(treeModel);
        treePromises.push(treePromise);
    });
